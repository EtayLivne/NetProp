...
def compare_multiple_rankings(file_paths):
    for rankings_file_path in file_paths:
        with open(rankings_file_path, 'r') as handler:
            rankings = json.load(handler)
        ...
        for dict_name, ranking_dict in rankings.items():
            ...
            top_l1_genes = [set.union(*[{poly_hash(top_l1[j]["knockout_gene_ids"])} for j in range(i + 1)]) for i in
                            range(len(top_l1))]
            top_l2_genes = [set.union(*[{poly_hash(top_l2[j]["knockout_gene_ids"])} for j in range(i + 1)]) for i in
                            range(len(top_l2))]
            l1_l2_commonality = [
                len(top_l1_genes[i].intersection(top_l2_genes[i])) / max(len(top_l1_genes[i]), len(top_l2_genes[i]))
                for i in range(len(top_l1))]
            file_name = "single" if "single" in rankings_file_path else "double"
            plt.plot(l1_l2_commonality, label=f'{file_name}: {dict_name}')




    plt.title("l1 and l2 distance and norm: commonality chart")
    plt.legend()
    plt.show()